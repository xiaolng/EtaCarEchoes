...
def get_LCdata(table, group=None, indices=None, excl_inds=None):
    ...
    if excl_inds is not None:
        inds_t = [w_i for w_i in inds_t if w_i not in excl_inds]
...
def get_LCinds(df, lc_ID, f_lm=None, ferr_lm=None, bad_expnums=None, bin_size=None, peak_range='full', diffstats=None):
    """
    ...
    df.t.loc[id_inds,'good_image'] = False
    if bad_expnums is not None:
        working_inds = []
        for i in id_inds:
            expnum = df.t.loc[i,'im_expnum']
            if expnum not in bad_expnums:
                working_inds.append(i)
        df.t.loc[working_inds, 'good_image'] = True
    ...
    if diffstats is not None:
        temp_inds = []
        for i, val in enumerate(df.t.loc[working_inds,'im_expnum']):
            temp_inds2 = np.where(diffstats.t['im_expnum'].eq(val))[0][0]
            if diffstats.t.loc[temp_inds2,'X2NRM00'] <= 15.0 and diffstats.t.loc[temp_inds2,'IM_M5SIGMA'] >= 21.0 and diffstats.t.loc[temp_inds2,'IM_FWHM'] <= 7.0:
                temp_inds.append(working_inds[i])
        working_inds = np.array(temp_inds)

    if bin_size is None:
        g_ixs = working_inds
        return(g_ixs)
        
    elif bin_size is 'gauss':
        ch_t, ch_mu, ch_std = get_LCfit(df,indices=working_inds, metric=5e4)
        ch_model = interp1d(ch_t,ch_mu)
        check_inds = np.where(abs(ch_model(df.t.loc[working_inds,'mjd'])-df.t.loc[working_inds,'muJyas2']) < 5.0)[0]
        g_ixs = working_inds[check_inds]
        # g_ixs = working_inds

        if peak_range is 'full':
            peak_loc = ch_t[np.argmax(ch_mu)]
        elif peak_range is None:
            peak_loc = -1
        elif len(peak_range) == 2:
            t_min = peak_range[0]
            t_max = peak_range[1]
            new_mu = ch_mu[(ch_t<t_max) & (ch_t>t_min)]
            new_t = ch_t[(ch_t<t_max) & (ch_t>t_min)]
            peak_loc = new_t[np.argmax(new_mu)]
        return(g_ixs, peak_loc)

    # elif bin_size is 'loop':
    #     test_inds = df.t.loc[working_inds,'mjd'] < 
    #     df.calcaverage_sigmacutloop('muJyas2', indices=test_inds, noisecol='muJyas2_err', percentile_cut_firstiteration=70, Nsigma=5.0)
...
def flag_inds(df, f_lm=None, ferr_lm=None, bad_expnums=None, bin_size=None, bad_ids=None, peak_range='full', diffstats=None):
    ...
    if bin_size is None:   
        g_ixs = [get_LCinds(df, lc_ID=i, f_lm=f_lm, ferr_lm=ferr_lm, bad_expnums=bad_expnums, bin_size=bin_size) for i in lc_ids]
        df.t.loc[np.concatenate(g_ixs),'g_ixs_flag'] = True
        
    elif bin_size is 'gauss':
        df.t['peak_loc'] = None
        for i in lc_ids:
            lcinds = get_LCinds(df, lc_ID=i, f_lm=f_lm, ferr_lm=ferr_lm, bad_expnums=bad_expnums, bin_size=bin_size, peak_range=peak_range, diffstats=diffstats)
            g_ixs = lcinds[0]
            peak_loc = lcinds[1]
            df.t.loc[g_ixs,'g_ixs_flag'] = True
            df.t.loc[g_ixs, 'peak_loc'] = peak_loc

    # elif bin_size is 'loop':
    #     for i in lc_ids:
    #         lcinds = get_LCinds(df, lc_ID=i, f_lm=f_lm, ferr_lm=ferr_lm, bad_expnums=bad_expnums, bin_size=bin_size, peak_range=peak_range)
    #         g_ixs = lcinds
    #         df.t.loc[g_ixs,'g_ixs_flag'] = True
...
def get_tshift_scale_offset(t1, f1, f1_err, t2, f2, f2_err, tshift_d):#, t_range=None, t_peaks=None, metric=None):
...
def fit_LCs(df, templ_ID=0, LCmodel=None, peak_range='full', tshift_val=None, tshift_ls=None):#, t_range=None, t_peaks=None, metric=None):
    '''
    ...
    if LCmodel is None:
        ...
        peak_loc1 = np.unique(df.t.loc[g1_inds,'peak_loc'])[0]
        ...
        if peak_range is 'full':
            peak_loc1 = t1[np.argmax(f1)]
        elif peak_range is None:
            peak_loc1 = -1
        elif len(peak_range) == 2:
            t_min = peak_range[0]
            t_max = peak_range[1]
            new_f = f1[(t1<t_max) & (t1>t_min)]
            new_t = t1[(t1<t_max) & (t1>t_min)]
            peak_loc1 = new_t[np.argmax(new_f)]
    ...
    for i, iid in enumerate(lc_ids):
        ...
        peak_loc2 = np.unique(df.t.loc[g2_inds,'peak_loc'])[0]

        if tshift_ls is not None: tshift_d = tshift_ls[i]
        elif tshift_val is not None: tshift_d = tshift_val

        if LCmodel is None:
            if tshift_val is None and tshift_ls is None: tshift_d = peak_loc2 - peak_loc1
            tshift, a, b = get_tshift_scale_offset(t1, f1, f1_err, t2, f2, f2_err, tshift_d=tshift_d)
        elif LCmodel is not None:
            if peak_range is None: tshift_d = t1[-1] - t2[-1]
            elif tshift_val is None and tshift_ls is None: tshift_d = peak_loc1 - peak_loc2
            tshift, a, b = get_tshift_scale_offset(t2, f2, f2_err, t1, f1, f1_err, tshift_d=tshift_d)
